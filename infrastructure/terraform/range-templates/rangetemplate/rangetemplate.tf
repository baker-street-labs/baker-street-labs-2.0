...
# - Folder: rangetemplate (configurable via variable)
...
data "vsphere_folder" "rangetemplate" {
  path          = var.vsphere_folder  # Folder syntax: range<name> (e.g., rangetemplate, rangexdr)
  datacenter_id = data.vsphere_datacenter.dc.id
}
...
# - Folder: rangetemplate (configurable)
...
#   folder           = data.vsphere_folder.rangetemplate.path
# 
#   num_cpus               = 2
#   memory                 = 4096
...
output "affinity_rule_id" {
  ...
  value       = vsphere_compute_cluster_vm_affinity_rule.rangetemplate_affinity.id
}
