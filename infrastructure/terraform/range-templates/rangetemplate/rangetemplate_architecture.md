...
| Folder Path | Configurable (default: rangetemplate) | Folder syntax: range<name> |
...
| Folder | Configurable (default: rangetemplate) | VM folder path |
...
**Terraform**:
```hcl
resource "vsphere_compute_cluster_vm_affinity_rule" "rangetemplate_affinity" {
  name                = "${var.vsphere_folder}-vm-affinity"
  compute_cluster_id  = data.vsphere_compute_cluster.cluster.id
  virtual_machine_ids = [for vm in vsphere_virtual_machine.rangetemplate_vms : vm.id]
...
The enhanced extraction script (`rangetemplate_extract.ps1`) extracts:
...
| Folder | string | Folder path | rangetemplate (configurable) |
...
| NetworkName | string | Network/port group name | e.g., rangetemplate-users (configurable) |
...
1. **Extraction**: Run enhanced `rangetemplate_extract.ps1` to extract:
   - VM configurations
   - Affinity rules
...
- `rangetemplate_design.md` - Mermaid diagrams for VM affinity, network topology, and deployment flow
- `rangetemplate.tf` - Terraform configuration with affinity rules
...
- **Folder Syntax**: Configurable (default: rangetemplate) - following range<name> pattern
